...
use CodeHuiter\Facilities\Exception\Runtime\AuthRuntimeException;
...
use CodeHuiter\Facilities\Service\ValidatedData;
...
class Mauth {
    ...
    //v3.6
    private function getPasswordRecoveryFacilities(){
        $subject = '['.$this->mm->app_properties['site_name'].'] '.lang('mauth.eFacilities.recpass.subject');;
        $str = lang('mauth.eFacilities.recpass.p11').' '. lang('mauth.eFacilities.recpass.p12') ."\n";
        $str .= lang('mauth.eFacilities.recpass.p2')."\n";
        $str .= ''.$this->mm->app_properties['site_url'].'/auth/recovery_password_email?user_id={#user_id}&token={#token}  '."\n";
        $str .= lang('mauth.eFacilities.recpass.p4')."\n";
    ...
    public function sendEmailConfirm($someInfo){
        ...
        $letter = $this->getConfirmEmailFacilities();
    ...
    public function sendPasswordRecovery($someInfo, $email = ''){
        ...
        if (!$token) return false;
        $letter = $this->getPasswordRecoveryFacilities();
        $this->queue_mailer->send($someInfo['email'], $letter['subject'], $this->mm->replacer($letter['text'], array(
            '{#user_id}' => $someInfo['id'],
            '{#login}' => $someInfo['login'],
            '{#token}' => $token
