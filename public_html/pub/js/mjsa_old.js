...
var mjsa = new (function ($){
	...
	this.bodyUpdate = function(){
		mthis.bodyLink(location.pathname+location.search,{nopush:true,callback:function(){}});
	};
	...
	this.bodyAjax.init = function(selector){
		...
		$(document).on('click', selector, function(){
			if (($(this).attr('href') !== '#'))
				mthis.bodyLink($(this).attr('href'),{el:this});
			return false;
		});
		...
		if ((window.history && history.pushState)){
			window.addEventListener("popstate", function(e) {
				...
				if (location.pathname+location.search !== mthis.currentPathname){
					mthis.bodyLink(location.pathname+location.search,{nopush:true,scrollto:e.state.scroll});//(e.url); not working :(
				}
				e.preventDefault();
			}, false);
		}
	...
	*/
	...
	this._upload = function(files_info,opt){
		...
		if (opt.callPre) opt.callPre({obj: files});
		http.open('POST', opt.url);
		if (opt.headers){
			var params = mthis.get(opt.headers);
			for (var key in params) http.setRequestHeader(key, params[key]);
		}
		http.send(form);
		return http;
	};
...
mjsa = (function ($){
	...
	this.scrollPopup = (function($){
		var defOptions = {
			...
			loadingBlock: '<img src="/pub/images/loader.gif" alt="loading"/>',
...
v1.1.4.106 and v1.1.4.107
scrollPopup hotfix

v1.1.3.105
form incorrectscroll
uploading multirequest fix

v1.1.3.104 (2015-11-23)
multirequestCallback after every downloaded file (with last)
scrollPopup processed mjsaCommands


v1.1.3.104 (2015-11-23)
upload add mode multirequests

v1.1.2.103
+ fix scrollPopup hide loading

v1.1.2.102 (2015-06-27)
+.isInWindow

v1.1.1.101 (2015-04-22)
scrollPopup some changes, style in mjsa.css, name
bodyAjax loadCollectedParams fixed

v1.1.0.100 (2015-02-21)
mjsa.css > .hint.hlefted - align hint to right
.get() now used in opt.params (.easilypostAjax, .upload, .authocomplete)
.printHint - some changes
.upload: remove error checking in callAfter (do it in callback)
unloadFunc in bodyAjax
.scrollPopup padding changes
def.hints.call -> def.hints.callback
new fontello font icons

v1.0.0.99 (2014-09-21)
bodyAjax fix
...
def.bodyAjax - inner param, switched on when bodyAjax.init called
...
fix: bodyUpdate in old browsers - reload.
...
rename = bodyAjax
	...
	html5AjaxBodyUpdate => bodyUpdate
